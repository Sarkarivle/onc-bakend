...
const importFromUrl = async (req, res) => {
  try {
    ...
    const jobData = { ...result.jobData, fullHtmlContent: result.htmlTemplate, applyLink: url, category: category || 'Jobs' };
...
const addJobFromJson = async (req, res) => {
  try {
    ...
    const jobData = { ...parsedData, category: category || parsedData.category || 'General' };
    const newJob = await Job.create(jobData);
    res.status(201).json({ status: 'success', data: newJob });
  } catch (err) {
    res.status(400).json({ status: 'fail', message: err.message });
...
module.exports = { getAllJobs, importFromUrl, addJobFromJson, getAiMatchAdvice, deleteJob };
