...
const THANKS_OK = ['theek hai', 'shukriya', 'ok thanks', 'acha ji', 'thank you', 'ok', 'good', 'nice', 'dhanyawad', 'thanks', 'samajh gaya', 'karo', 'haan', 'dikhao', 'zaroor', 'yes', 'do it'];
const AMBIGUOUS_SHORT = ['naukri', 'fees?', 'form?', 'apply?', 'form kaise bharein?', 'details', 'yojna', 'form', 'apply', "don't know", 'kuch aur', 'batao', 'phir se'];
...
const ADVICE_KEYWORDS = ['kare', 'karun', 'karu', 'kaise', 'kyun', 'kyu', 'percent', 'chance', 'probability', 'vs', 'better', 'guide', 'suggestion', 'advice', 'kya', 'kaun', 'kaunsi', 'list'];
const FOLLOWUP_PRONOUNS = ['iska', 'uska', 'isme', 'usme', 'ye', 'wo', 'wahi', 'phir', 'aur'];
...
class DeterministicIntentResolver {
    static resolve(query) {
        ...
        if (
            (normalizedQuery.length < 20 && GREETINGS.some(g => normalizedQuery.startsWith(g) || normalizedQuery.endsWith(g))) ||
            /^(hi|hello|hey|namaste)\s+jobo\b/.test(normalizedQuery)
        ) {
             return this._createResponse({
                intent: 'GREETING',
                behavior: 'GREET',
        ...
        // Rule: Only auto-resolve if it's NOT an advice/question query or a follow-up.
        if (
            !ADVICE_KEYWORDS.some(term => normalizedQuery.includes(term)) &&
            !FOLLOWUP_PRONOUNS.some(term => normalizedQuery.includes(term)) &&
