/**
 * StrongIntentResolver Module
 * Responsibility: Runs before generic field-detail detection so independent domains win.
 */
...
class StrongIntentResolver {
    static resolve(text = "", ruleResult = {}) {
        ...
        const hasJobAsk = /(job|jobs|vacancy|vacancies|bharti|naukri|kaam|rojgar|form|apply|ke liye|dikhao|batao|hai kya|chahiye|jankari| kab |kab tak|notification|recruitment|update|updates|details|year|date|opening|listing|in\s+\w+|in bihar|in up|in delhi|in madhya pradesh|in rajasthan|in state)/.test(q);
