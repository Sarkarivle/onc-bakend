/**
 * SemanticRouter Module (Production Grade)
...
class SemanticRouter {
    ...
    /**
     * Thread-safe initialization using a singleton promise.
     */
    static async init() {
        ...
        this.initPromise = (async () => {
            logger.info("🚀 Initializing Semantic Router (Turbo Mode)...");
            try {
                const intentEntries = Object.entries(CONFIG.INTENT_ANCHORS);

                // Parallelize all intent vector generation
                await Promise.all(intentEntries.map(async ([intent, examples]) => {
                    const vectors = await Promise.all(
                        examples.map(async (ex) => {
                            try {
                                return await VectorService.generate(ex);
                            } catch (e) {
                                logger.error(`Failed to generate vector for anchor: ${ex}`, e);
                                return null;
                            }
                        })
                    );
                ...
                logger.error("❌ Critical: Semantic Router initialization failed", error);
                this.initPromise = null; // Reset to allow retry on next call
                throw error;
            }
        })();
    ...
    static async route(query, state = {}) {
        try {
            ...
            const lastAiAskedQuestion = lastAssistantMessage.includes('?') ||
                                         /karu|dikhau|bataun/i.test(lastAssistantMessage);

            // --- STEP 1: PIVOT CHECK ---
            const rawQueryVector = await VectorService.generate(query);
            const topRawIntent = this._getBestMatch(rawQueryVector);

            if (topRawIntent && topRawIntent.score > CONFIG.THRESHOLDS.PIVOT) {
                return this._buildMatch(topRawIntent.intent, topRawIntent.score, "Intent Pivot", currentTopic);
            }

            // --- STEP 2: CONTEXTUAL STATE MACHINE ---
            if (isShortQuery && lastAiAskedQuestion) {
                ...
                if (affirmative.some(a => lowerQuery.includes(a))) {
                    const likelyIntent = (currentTopic === 'JOB_SEARCH' || /job/i.test(lastAssistantMessage)) ? 'JOB_SEARCH' : 'CONTINUE';
                    return this._buildMatch(likelyIntent, CONFIG.THRESHOLDS.AFFIRMATIVE, "State Transition", currentTopic);
                }
            }

            // --- STEP 3: CONTEXT-ENRICHED MATCHING ---
            ...
            if (topEnrichedIntent && topEnrichedIntent.score > CONFIG.THRESHOLDS.ENRICHED) {
                return this._buildMatch(topEnrichedIntent.intent, topEnrichedIntent.score, "Enriched Match", currentTopic);
            }

            return null;
        } catch (error) {
            logger.error("Error in SemanticRouter.route:", error);
            return null; // Graceful fallback to LLM Planner
        }
    }
    ...
    /**
     * Builds a Cognitive Map compatible output
     */
    static _buildMatch(intent, score, reasoning, currentTopic) {
        logger.info(`⚡ Semantic Route: ${intent} (${reasoning}) | Score: ${score.toFixed(3)}`);

        return {
            intent: intent,
            confidence: score,
            domain: this._getDomain(intent),
            goal: `Execute student request for ${intent}`,
            topic: currentTopic,
            needsPlanning: false,
    ...
    static _getDomain(intent) {
        const mapping = {
            'JOB_SEARCH': 'GOVERNMENT_JOBS',
            'FIELD_DETAILS': 'GOVERNMENT_JOBS',
            'ORDINAL_FOLLOWUP': 'GOVERNMENT_JOBS',
            'CONTINUE': 'GOVERNMENT_JOBS',
            'CAREER_GUIDANCE': 'CAREER_GUIDANCE',
            'MOTIVATION': 'CAREER_GUIDANCE',
            'RESUME': 'STUDENT_UTILITIES',
            'PROFILE_INQUIRY': 'GENERAL_HELP'
        };
    ...
    static _getDefaultExecution(intent) {
        ...
        if (['JOB_SEARCH', 'ORDINAL_FOLLOWUP', 'CONTINUE', 'FIELD_DETAILS'].includes(intent)) {
            return [...base, { step: 2, tool: "RAG", purpose: "search" }, { step: 3, tool: "LLM", purpose: "synthesis" }];
        }
        if (['PROFILE_INQUIRY', 'MOTIVATION'].includes(intent)) {
            return [...base, { step: 2, tool: "PROFILE", purpose: "fetch_user_profile" }, { step: 3, tool: "LLM", purpose: "synthesis" }];
        }
