/**
 * ResponsePlanner Module (Formerly Planner)
 * Responsibility: Decide action, mode, and data source policy.
 */
...
class ResponsePlanner {
    static plan(query, resolvedIntent, state = {}, profile = {}) {
        ...
        let mode = this._decideMode(primary, domain, resolvedIntent, query);
    ...
    static _shouldUseContext(q, resolvedIntent, state) {
        const newTopicRegex = /\b(police kaise bane|teacher kaise bane|engineer kaise bane|career|12th ke baad|10th ke baad|graduation ke baad|course|diploma|iti ke baad|mujhe .* banna hai|doctor|mbbs|nursing|medical career)\b/i;
        ...
        return Boolean(resolvedIntent.isFollowUp);
    }
    ...
    static _decideMode(primary, domain, resolvedIntent, query) {
        ...
        if (primary === 'CAREER_GUIDANCE' || domain === 'CAREER' || q.includes('doctor') || q.includes('mbbs') || q.includes('nursing')) return 'CAREER_GUIDANCE';
